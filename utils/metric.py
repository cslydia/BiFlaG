...
def get_region_fmeasure(p_entities, r_entities, label_alphabet, output_path):
    """
    """
    elist = [v for k, v in label_alphabet.id_to_entity.items() if v != 'O']
    result = [[0] * 3] * len(elist)
    file = open(output_path, 'w')

    for p_tags, r_tags in zip(p_entities, r_entities):

        file.write('predict|' + '|'.join(
            [','.join([str(entity[0]), str(entity[1]), entity[2]]) for entity in p_tags]) + '\n')
        file.write('gold|' + '|'.join(
            [','.join([str(entity[0]), str(entity[1]), entity[2]]) for entity in r_tags]) + '\n\n')

        result, p_tags, r_tags = eval_level(r_tags, p_tags, result, elist)
    ...
    tab.header(headings)
    overall_p, overall_r, overall_f = measures(result, elist, tab)

    file.close()
    print(tab.draw())

    return round(overall_p, 4), round(overall_r, 4), round(overall_f, 4)
...
def measures(lst, e_lst, table):
    ...
    return overall_p, overall_r, overall_f
...
def get_ner_fmeasure(golden_lists, predict_lists, label_type="BMES"):
    ...
    return accuracy, precision, recall, f_measure, golden_num, predict_num, right_num

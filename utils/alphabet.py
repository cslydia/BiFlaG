...
class Alphabet:
    ...
    def update_dict(self):
        dico = dict((v, k.split('-')[1]) for k, v in self.instance2index.items() if k.split('-')[0] == 'B')
        
        self.id_to_entity = dict((i + 1, v) for i, (k, v) in enumerate(dico.items()))
        self.entity_to_id = dict((v, i+1) for i, (k, v) in enumerate(dico.items()))
        self.id_to_entity[0] = 'O'
        ...
        self.instance2index = dict((v, k+1) for k, v in id_to_tag.items())
